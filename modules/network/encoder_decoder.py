...
class Critic(nn.Module):
    def __init__(self, ndf, ngf, z_dim, critic_dim, input_size: tuple = (3, 32, 32), output_decoder=False, **kwargs):
        ...
        self.encoder = Encoder(self.image_size, n_channel=self.n_channel, k=self.critic_dim, ndf=self.ndf)
        self.decoder = Decoder(self.image_size, n_channel=self.n_channel, k=self.z_dim, ngf=self.ngf)
    ...
    def forward(self, input_data):
        ...
        if not self.output_decoder:
            return f_enc_X.view(input_data.shape[0], -1)
        else:
            f_dec_X = self.decoder(f_enc_X)
            # f_dec_X = f_dec_X.view(input_data.shape[0], self.n_channel, self.image_size, self.image_size)
            return f_enc_X.view(input_data.shape[0], -1), f_dec_X.view(input_data.shape[0], -1)
